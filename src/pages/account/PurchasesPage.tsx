...
import { Star, Key, Check } from 'lucide-react'
...
export default function PurchasesPage() {
  ...
  return (
    <div className="flex justify-center px-4 sm:px-6 lg:px-8 py-10">
      ...
      const keyCount = order.orderItems.reduce((total, item) => total + item.quantity, 0);
      const products = orderProducts[order.id];
          ...
          <div className="flex flex-col text-left">
            <div className='flex flex-col font-semibold text-white text-sm sm:text-base'>
              ...
              <span className="text-gray-400 text-xs">
                ({count} {getGameWord(count)}, {keyCount} {getKeyWord(keyCount)})
              </span>
            </div>
          </div>
              ...
              >
                ...
                {order.status === 'COMPLETED'
                  ...
                  : 'Odbierz klucze'}
                          ...
                          {!['PAYMENT_FAILED', 'CANCELLED', 'READY_FOR_PAYMENT', 'PAID'].includes(order.status) && (
                            !order.hasReview ? (
                              <Button
                                size="sm"
                                className="bg-[#D4A44A] text-black hover:bg-[#B8873D] text-xs sm:text-sm border-[#D4A44A]"
        ...
        <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
          <DialogContent className="bg-[#1F1F1F] border-[#3A3A3A] text-white max-w-md">
            ...
            <div className="space-y-3 my-4">
              ...
              <div className="flex items-start gap-2 text-sm text-gray-300">
                <Check className="h-5 w-5 text-[#D4A44A] flex-shrink-0 mt-0.5" />
                <span>Posiadasz konta na platformach, do których zakupiłeś gry</span>
              </div>
            </div>

            <div className="bg-[#2A2A2A] border border-[#3A3A3A] rounded-lg p-3 text-xs text-gray-400">
              <strong className="text-yellow-500">Uwaga:</strong> Po odebraniu kluczy nie będzie możliwości zwrotu zamówienia.
            </div>
